...
function ECA_kinetics!(ECA::Matrix{T}, S::Vector{T}, E::Vector{T}, K::Matrix{T}, k2p::Matrix{T}) where {T<:Real}
    (I,J) = size(K)
    @assert I == size(S,1)
    @assert J == size(E,1)
    @assert (I,J) == size(k2p)
    ...
    @inbounds for i in 1:I, j in 1:J
            val = k2p[i,j]*E[j]*(S[i]/K[i,j])/(1 + F_r[i] + F_c[j])
            ECA[i,j] = val[1]
    end

    any(x->x==true, isnan.(ECA)) ? throw(DomainError("NaN in DEBJulia.ECA_kinetics!")) : return ECA
end
...
function ECA_kinetics!(ECA::Matrix{T}, S::Vector{T}, E::Vector{T}, K::Matrix{T}, k2p::Matrix{T}, α_kin::Matrix{T}, t::String) where {T<:Real}
    (I,J) = size(ECA)
    @assert I == size(S,1)
    @assert J == size(E,1)
    @assert (I,J) == size(k2p)
    ...
    @inbounds for i in 1:I, j in 1:J
            val = k2p[i,j]*E[j]*(α_kin[i,j]*S[i]/K[i,j])/(1 + F_r[i] + F_c[j])
            ECA[i,j] = val[1]
    end
